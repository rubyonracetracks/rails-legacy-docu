---
id: 02-05
title: "Unit 2 Chapter 5: Build Script"
sidebar_label: "Chapter 5: Build Script"
---
...
## Git Branch
Enter the command "git checkout ruby_on_racetracks".  This returns you to the ruby_on_racetracks branch.

## Rails Neutrino Source Code
* Use the SearchMonkey tool to search for the word "WICHTIG/LEGACY/bash".  This shows you the elements from the Rails Neutrino app generator that you should add to legacy apps.
* For this chapter, focus on just the contents of the mod-01-02.rb script.

## Adding the Scripts
* Copy the Bash scripts mentioned in the "WICHTIG/LEGACY/bash" note to the root directory of the Rails app.  Remove the "mod-01-02-" portions from the beginning of the script names.  These Bash scripts belong in the root directory of the Rails app.
...
## Local Web Server
* Go to the root directory of the app, and enter the command "sh server.sh".
...
## Test Suite
* If you haven't already done so, create a new tab in LXTerminal, enter the command "sh join.sh", and cd your way into the app.
* Check the Gemfile to see what test suite is in use.  It may be MiniTest, RSpec, Cucumber, or a combination.
* The test_app.sh script is configured with the assumption that MiniTest is the one and only test suite used.  If something else is used in addition to MiniTest, you need to add additional commands to run the other test suites.  If something else (like RSpec) is used instead of MiniTest, you need to replace the "rails test" command.
...
* Go to Docker Hub and build the custom Docker image.
...
* If necessary, enter the following commands:
```
git add .
git commit -m "Shortened the build_fast.sh script"
git push origin ruby_on_racetracks
```
